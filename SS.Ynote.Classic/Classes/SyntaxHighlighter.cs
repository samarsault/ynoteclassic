...
//
// Copyright (C) 2013-14 Samarjeet Singh for the Ynote Classic Project
// 
//
...
namespace SS.Ynote.Classic
{
    ...
    public sealed class SyntaxHighlighter : ISyntaxHighlighter
    {
        ...
        private Regex ObjCKeywordRegex2;
        ...
        /// </summary>
        /// <param name="r"></param>
        private void CSSHighlight(Range r)
        {
            r.tb.CommentPrefix = "/*^";
            r.ClearStyle(StringStyle, CSSPropertyStyle, CSSSelectorStyle,CSSPropertyValueStyle, NumberStyle);

            r.tb.RightBracket = '}';
            r.tb.LeftBracket2 = '(';
            r.tb.RightBracket2 = ')';
            r.tb.LeftBracket = '{';
            var visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentTagStyle);
            if (_cssCommentRegex2 == null)
                InitCssRegex();
            visiblerange.SetStyle(CommentTagStyle, _cssCommentRegex2);
            visiblerange.SetStyle(CommentTagStyle, _cssCommentRegex3);
            r.SetStyle(CSSPropertyStyle, _cssPropertyRegex);
            r.SetStyle(CSSSelectorStyle, _cssSelectorRegex);
            r.SetStyle(CSSPropertyValueStyle, _cssPropertyValueRegex);
            r.SetStyle(NumberStyle, _cssNumberRegex);
            r.ClearFoldingMarkers();
            r.SetFoldingMarkers("{", "}");
            r.SetFoldingMarkers(@"/\*", @"\*/"); //allow to collapse comment block
        }

        private void BatchSyntaxHighlight(Range r)
        {
        ...
        /// </summary>
        /// <param name="range"></param>
        private void JavaSyntaxHighlight(Range range)
        {
            ...
            var visiblerange = range.tb.VisibleRange;
            if (_javaStringRegex == null)
                InitJavaAsRegex();
            visiblerange.ClearStyle(CommentStyle);
            range.ClearStyle(PreprocessorStyle, StringStyle, NumberStyle, ClassNameStyle, ClassNameStyle2, KeywordStyle,
                KeywordStyle2, KeywordStyle3, CharStyle);
            visiblerange.SetStyle(CommentStyle, _javaCommentRegex1);
            visiblerange.SetStyle(CommentStyle, _javaCommentRegex2);
            visiblerange.SetStyle(CommentStyle, _javaCommentRegex3);
            //preprocessor
            range.SetStyle(PreprocessorStyle, @"#.*$", RegexOptions.Multiline);
            //string
            range.SetStyle(StringStyle, _javaStringRegex);
            //number highlighting
            range.SetStyle(NumberStyle, _javaNumberRegex);
            //attribute highlighting
            range.SetStyle(CommentStyle, _javaAttributeRegex);
            //class name highlighting
            range.SetStyle(ClassNameStyle, _javaClassNameRegex);
            range.SetStyle(ClassNameStyle2, _javaFunctionRegex);
            //keyword highlighting
            range.SetStyle(KeywordStyle, _javaKeywordRegex);
            range.SetStyle(KeywordStyle2, _javaKeywordRegex2);
            range.SetStyle(KeywordStyle3,
                new Regex(
        ...
        private void QBHighlight(Range r)
        {
            ...
            r.tb.RightBracket = ')';
            r.tb.RightBracket2 = ']';
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle, KeywordStyle2, VariableStyle, NumberStyle);
        ...
        /// </summary>
        /// <param name="r"></param>
        private void CPPSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "//";
            r.tb.LeftBracket = '(';
            r.tb.LeftBracket2 = '{';
            ...
            var visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle);
            r.ClearStyle(StringStyle, KeywordStyle, KeywordStyle2, NumberStyle, CharStyle, ClassNameStyle,
                ClassNameStyle2, PreprocessorStyle);
            if (_cppCommentRegex1 == null)
                InitCPPRegex();
            visiblerange.SetStyle(CommentStyle, _cppCommentRegex1);
            visiblerange.SetStyle(CommentStyle, _cppCommentRegex2);
            visiblerange.SetStyle(CommentStyle, _cppCommentRegex3);
            r.SetStyle(StringStyle, _cppStringRegex);
            r.SetStyle(PreprocessorStyle, "#.*$", RegexOptions.Multiline);
            r.SetStyle(ClassNameStyle, _cppClassNameRegex);
            r.SetStyle(ClassNameStyle2, _cppFunctionsRegex);
            r.SetStyle(KeywordStyle, _cppKeywordRegex);
            r.SetStyle(KeywordStyle2, _cppKeywordRegex2);
            r.SetStyle(NumberStyle, _cppNumberRegex);
            r.SetStyle(CharStyle, @"\;|-|>|<|=|\+|\,|\$|\^|\[|\]|\$|\!|\?");
            r.ClearFoldingMarkers();
        ...
        private void XmlSyntaxHighlight(Range range)
        {
            ...
            range.tb.LeftBracket2 = '(';
            range.tb.RightBracket2 = ')';
            Range visiblerange = range.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle);
            //clear style of changed range
            range.ClearStyle(KeywordStyle, TagBracketStyle, TagNameStyle, AttributeStyle, AttributeValueStyle,
                HtmlEntityStyle);
            //
            ...
            //comment highlighting
            visiblerange.SetStyle(CommentStyle, _htmlCommentRegex1);
            visiblerange.SetStyle(CommentStyle, _htmlCommentRegex2);
            //tag brackets highlighting
            range.SetStyle(TagBracketStyle, _htmlTagRegex);
            //tag name
            range.SetStyle(TagNameStyle, _htmlTagNameRegex);
            //end of tag
            range.SetStyle(TagNameStyle, _htmlEndTagRegex);
            //attributes
            range.SetStyle(AttributeStyle, _htmlAttrRegex);
            //attribute values
            range.SetStyle(AttributeValueStyle, _htmlAttrValRegex);
            //html entity
            range.SetStyle(HtmlEntityStyle, _htmlEntityRegex);
        ...
        private void PythonSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "#";
            r.tb.LeftBracket = '(';
            r.tb.RightBracket = ')';
            ...
            r.tb.RightBracket2 = ']';
            var visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle, CommentTagStyle);
            r.ClearStyle(CommentStyle, StringStyle, ClassNameStyle, ClassNameStyle2, KeywordStyle, KeywordStyle2,
                NumberStyle, CharStyle);
            visiblerange.SetStyle(CommentTagStyle, "(\"\"\".*?\"\"\")|(.*\"\"\")|(\'\'\'.*?\'\'\')|(.*\'\'\')",
                RegexOptions.Singleline | RegexOptions.RightToLeft | RegexOptions.Multiline);
            visiblerange.SetStyle(CommentStyle, "#.*$", RegexOptions.Multiline);
            r.SetStyle(StringStyle, new Regex(
                @"
                            # Character definitions:
                            '
                            (?> # disable backtracking
        ...
        private void FSharpSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "//";
            r.tb.LeftBracket = '(';
            r.tb.LeftBracket2 = '[';
            r.tb.RightBracket = ')';
            r.tb.RightBracket2 = ']';
            Range visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle);
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle, KeywordStyle2);
            visiblerange.SetStyle(CommentStyle, "//.*$");
            visiblerange.SetStyle(CommentStyle, @"(\(\*.*?\*\))|(\(\*.*)");
            r.SetStyle(StringStyle, @"""""|@""""|''|@"".*?""|(?<!@)(?<range>"".*?[^\\]"")|'.*?[^\\]'");
            r.SetStyle(KeywordStyle,
        ...
        /// </summary>
        /// <param name="range"></param>
        private void IniSyntaxHighlight(Range range)
        {
            range.tb.CommentPrefix = ";";
        ...
        /// </summary>
        /// <param name="r"></param>
        private void LuaSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "--";
            r.tb.LeftBracket = '(';
            r.tb.LeftBracket2 = '[';
            ...
            Range visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle);
            r.ClearStyle(StringStyle, KeywordStyle, KeywordStyle2, KeywordStyle3, VariableStyle, NumberStyle, CharStyle);
            visiblerange.SetStyle(CommentStyle, @"--.*$", RegexOptions.Multiline);
            visiblerange.SetStyle(CommentStyle, @"\-\-\[\[.+?\-\-\]\]",
            ...
            r.SetStyle(KeywordStyle2, @"\b(false|nil|true|coroutine|debug|io|math|os|package|string|table)\b");
            r.SetStyle(KeywordStyle3,
            ...
            r.SetStyle(CharStyle, @"\[|\]|\*|\?|\(|\)|\^|\!|\;|\,|\.|\:");
            r.ClearFoldingMarkers();
            r.SetFoldingMarkers(@"function", @"end function");
        }

        private void RubySyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "#";
            r.tb.LeftBracket = '(';
            r.tb.LeftBracket2 = '[';
            ...
            Range visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle);
            r.ClearStyle(CommentStyle, KeywordStyle, StringStyle, NumberStyle, ClassNameStyle, CharStyle,
                ClassNameStyle2, VariableStyle);
            visiblerange.SetStyle(CommentStyle, @"#.*$", RegexOptions.Multiline);
            visiblerange.SetStyle(CommentStyle, @"(=begin.*?=end)|(.*=end)",
            ...
            r.SetStyle(KeywordStyle2, @"\b(self|puts|true|false|nil)\b");
            r.SetStyle(NumberStyle, @"\b\\d+[\\.]?\\d*([eE]\\-?\\d+)?[lLdDfF]?\b|\b0x[a-fA-F\\d]+\b");
            r.SetStyle(CharStyle, @"\[|\]|\*|\?|\(|\)|\^|\!|\;|\,|\.|\:");
            PythonFold(r.tb);
        }

        private void JSONSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "//";
            r.tb.LeftBracket = '(';
            r.tb.RightBracket = ')';
            ...
            r.SetStyle(CommentStyle, new Regex(@"//.*$"));
        ...
        private void LispSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = ";";
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle, NumberStyle, CharStyle, ClassNameStyle2);
            r.SetStyle(CommentStyle, @"\;.*$", RegexOptions.Multiline);
        ...
        /// </summary>
        /// <param name="r"></param>
        private void PerlSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "#";
            r.tb.LeftBracket = '(';
            r.tb.RightBracket = ')';
            ...
            Range visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle);
            r.ClearStyle(StringStyle, VariableStyle, KeywordStyle, KeywordStyle2, KeywordStyle3, NumberStyle);
            r.SetStyle(StringStyle, @"""""|''|"".*?[^\\]""|'.*?[^\\]'", RegexCompiledOption);
            visiblerange.SetStyle(CommentStyle, "#.*$", RegexOptions.Multiline);
            visiblerange.SetStyle(CommentStyle, @"(=begin.*?=cut)|(.*=cut)",
                RegexOptions.IgnoreCase | RegexOptions.RightToLeft | RegexOptions.Singleline);
            r.SetStyle(VariableStyle, @"\$[a-zA-Z_\d]*\b|%[a-zA-Z_\d]*\b", RegexCompiledOption);
            r.SetStyle(KeywordStyle,
        ...
        private void PHPSyntaxHighlight(Range range)
        {
            ...
            visiblerange.ClearStyle(CommentStyle);
            //clear style of changed range
            range.ClearStyle(NumberStyle, VariableStyle, KeywordStyle2, KeywordStyle, KeywordStyle3, CharStyle);
            //
            if (_phpStringRegex == null)
                InitPHPRegex();
            //comment highlighting
            visiblerange.SetStyle(CommentStyle, _phpCommentRegex1);
            visiblerange.SetStyle(CommentStyle, _phpCommentRegex2);
            visiblerange.SetStyle(CommentStyle, _phpCommentRegex3);
            //string highlighting
            range.SetStyle(StringStyle, _phpStringRegex);
            //number highlighting
            range.SetStyle(NumberStyle, _phpNumberRegex);
            //var highlighting
            range.SetStyle(VariableStyle, _phpVarRegex);
            //keyword highlighting
            range.SetStyle(KeywordStyle2, _phpKeywordRegex1);
            range.SetStyle(KeywordStyle, _phpKeywordRegex2);
            range.SetStyle(KeywordStyle3, _phpKeywordRegex3);
            range.SetStyle(CharStyle, @">|\*|-|=|\+|\!|<|\^|\?");
        ...
        private void HighlightPHPHtml(Range range)
        {
            HTMLSyntaxHighlight(range);
            foreach (Range r in range.tb.GetRanges(@"(<\?.*?.*?\?>)", RegexOptions.Singleline))
            {
                //remove HTML highlighting from this fragment
                r.ClearStyle(StyleIndex.All);
                //do PHP highlighting
                PHPSyntaxHighlight(r);
            }
        }

        private void HighlightHtmlJSCSS(Range range)
        {
            HTMLSyntaxHighlight(range);
            foreach (Range r in range.tb.GetRanges(@"(<script.*?>.*?</script>)", RegexOptions.Singleline))
            {
                //remove HTML highlighting from this fragment
                r.ClearStyle(CommentStyle, AttributeStyle, AttributeValueStyle, HtmlEntityStyle);
                //do Jscript highlighting
                JScriptSyntaxHighlight(r);
            }
            foreach (Range r in range.tb.GetRanges(@"(<style.*?>.*?</style>)", RegexOptions.Singleline))
            {
                //remove HTML highlighting from this fragment
                r.ClearStyle(CommentStyle, AttributeStyle, AttributeValueStyle, HtmlEntityStyle);
                //do CSS highlighting
        ...
        private void InitCSharpRegex()
        {
            ...
            _cSharpKeywordRegex =
                new Regex(
                    ...
                    RegexCompiledOption);
            _cSharpKeywordRegex2 = new Regex(@"\b(void|bool|string|int|double|float|byte|uint|ushort|ulong)\b");
        ...
        private void CSharpSyntaxHighlight(Range range)
        {
            ...
            if (_cSharpStringRegex == null)
                InitCSharpRegex();
            //comment highlighting
            visiblerange.SetStyle(CommentStyle, _cSharpCommentRegex1);
            visiblerange.SetStyle(CommentStyle, _cSharpCommentRegex2);
            visiblerange.SetStyle(CommentStyle, _cSharpCommentRegex3);
            //string highlighting
            range.SetStyle(StringStyle, _cSharpStringRegex);
            //number highlighting
            range.SetStyle(NumberStyle, _cSharpNumberRegex);
            //attribute highlighting
            range.SetStyle(AttributeStyle, _cSharpAttributeRegex);
            //class name highlighting
            range.SetStyle(ClassNameStyle, _cSharpClassNameRegex);
            //funtion highlight
            range.SetStyle(ClassNameStyle2, _csharpFunctionRegex);
            //keyword highlighting
            range.SetStyle(KeywordStyle, _cSharpKeywordRegex);
            //keywordstyle2
            range.SetStyle(KeywordStyle2, _cSharpKeywordRegex2);
            range.ClearFoldingMarkers();
            range.SetFoldingMarkers("{", "}"); //allow to collapse brackets block
        ...
        private void InitJScriptRegex()
        {
            ...
            _jScriptKeywordRegex =
                new Regex(
                    @"\b(true|false|break|case|catch|const|continue|default|delete|do|escape|unescape|else|export|for|function|if|in|instanceof|new|null|return|switch|throw|try|var|void|while|with|typeof)\b",
                    RegexCompiledOption);
            _jScriptKeywordRegex2 =
                new Regex(
                    @"\b(Array|Boolean|Date|Math|Object|String|eval|isFinite|isNaN|parseInt|parseFloat|this|document|NaN|prototype)\b");
        ...
        private void JScriptSyntaxHighlight(Range range)
        {
            ...
            visiblerange.ClearStyle(CommentStyle);
            //clear style of changed range
            range.ClearStyle(CommentStyle, StringStyle, NumberStyle, KeywordStyle, KeywordStyle2, ClassNameStyle,
                ClassNameStyle2, CharStyle);

            if (_jScriptStringRegex == null)
                InitJScriptRegex();
            //comment highlighting
            visiblerange.SetStyle(CommentStyle, _jScriptCommentRegex1);
            visiblerange.SetStyle(CommentStyle, _jScriptCommentRegex2);
            visiblerange.SetStyle(CommentStyle, _jScriptCommentRegex3);
            //string highlighting
            range.SetStyle(StringStyle, _jScriptStringRegex);
            //number highlighting
            range.SetStyle(NumberStyle, _jScriptNumberRegex);
            range.SetStyle(ClassNameStyle, _jScriptFunctionRegex);
            range.SetStyle(ClassNameStyle2, _jScriptFunctionRegex2);
            //keyword highlighting
            range.SetStyle(KeywordStyle, _jScriptKeywordRegex);
            range.SetStyle(KeywordStyle2, _jScriptKeywordRegex2);
            range.SetStyle(CharStyle, @"\;|\,|<|>|-|\$|=|\!|\.|\?|\*|\&|\#|\^");
            //clear folding markers
            range.ClearFoldingMarkers();
            //set folding markers
        ...
        private void VBSyntaxHighlight(Range range)
        {
            ...
            //clear style of changed range
            range.ClearStyle(CommentStyle, StringStyle, CommentStyle, NumberStyle, ClassNameStyle, KeywordStyle);
            //
            if (_vbStringRegex == null)
                InitVBRegex();
            //string highlighting
            range.SetStyle(StringStyle, _vbStringRegex);
            //comment highlighting
            range.SetStyle(CommentStyle, _vbCommentRegex);
            //number highlighting
            range.SetStyle(NumberStyle, _vbNumberRegex);
            //class name highlighting
            range.SetStyle(ClassNameStyle, _vbClassNameRegex);
            //keyword highlighting
            range.SetStyle(KeywordStyle, _vbKeywordRegex);
        ...
        private void SQLSyntaxHighlight(Range range)
        {
            ...
            if (_sqlStringRegex == null)
                InitSQLRegex();
            //comment highlighting
            visiblerange.SetStyle(CommentStyle, _sqlCommentRegex1);
            visiblerange.SetStyle(CommentStyle, _sqlCommentRegex2);
            visiblerange.SetStyle(CommentStyle, _sqlCommentRegex3);
            //string highlighting
            range.SetStyle(StringStyle, _sqlStringRegex);
            //number highlighting
            range.SetStyle(NumberStyle, _sqlNumberRegex);
            //types highlighting
            range.SetStyle(TypesStyle, _sqlTypesRegex);
            //var highlighting
            range.SetStyle(VariableStyle, _sqlVarRegex);
            //statements
            range.SetStyle(StatementsStyle, _sqlStatementsRegex);
            //keywords
            range.SetStyle(KeywordStyle, _sqlKeywordsRegex);
            //functions
            range.SetStyle(FunctionsStyle, _sqlFunctionsRegex);
        ...
        private void DSyntaxHighlight(Range r)
        {
            ...
            Range visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle);
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle, KeywordStyle2, NumberStyle);
            visiblerange.SetStyle(CommentStyle, @"//.*$");
            visiblerange.SetStyle(CommentStyle,
        ...
        private void PascalSyntaxHighlight(Range r)
        {
            ...
            visiblerange.SetStyle(CommentStyle,
                new Regex(@"\(\*(.*?)\*\)", RegexOptions.Singleline | RegexCompiledOption));
            visiblerange.SetStyle(CommentStyle, @"({.*?})|({.*)", RegexOptions.Singleline);
            visiblerange.SetStyle(CommentStyle, @"({.*?})|(.*})", RegexOptions.Singleline | RegexOptions.RightToLeft);
        ...
        private void ScalaSyntaxHighlight(Range r)
        {
            ...
            Range visiblerange = r.tb.VisibleRange;
            visiblerange.ClearStyle(CommentStyle);
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle, KeywordStyle2, KeywordStyle3, NumberStyle, CharStyle);
            visiblerange.SetStyle(CommentStyle, CommentRegex1);
            visiblerange.SetStyle(CommentStyle, CommentRegex2);
            visiblerange.SetStyle(CommentStyle, CommentRegex3);
            r.SetStyle(StringStyle, new Regex(@"""""|''|"".*?[^\\]""|'.*?[^\\]'"));
        ...
        private void ObjCHighlight(Range r)
        {
            r.tb.CommentPrefix = "//";
            r.tb.LeftBracket = '(';
            r.tb.RightBracket = ')';
            ...
            r.tb.RightBracket2 = ']';
            Range visiblerange = r.tb.VisibleRange;
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle, KeywordStyle2, PreprocessorStyle, NumberStyle);
            visiblerange.ClearStyle(CommentStyle);
            if (_objCCommentRegex1 == null)
                InitObjCRegex();
            visiblerange.SetStyle(CommentStyle, _objCCommentRegex1);
            visiblerange.SetStyle(CommentStyle, _objCCommentRegex2);
            visiblerange.SetStyle(CommentStyle, _objCCommentRegex3);
            r.SetStyle(PreprocessorStyle, _objCPreprocessorRegex);
            r.SetStyle(StringStyle, _objCStringRegex2);
            r.SetStyle(StringStyle, _objCStringRegex);
            r.SetStyle(ClassNameStyle, _objCClassNameRegex);
            r.SetStyle(ClassNameStyle2, _objCFunctionsRegex);
            r.SetStyle(KeywordStyle, _objCKeywordRegex);
            r.SetStyle(KeywordStyle2, ObjCKeywordRegex2);
            r.SetStyle(NumberStyle, _objCNumberRegex);
            r.ClearFoldingMarkers();
            r.SetFoldingMarkers("{", "}");
            r.SetFoldingMarkers(@"/\*", @"\*/"); //allow to collapse comment block
        }

        private void ActionscriptSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "//";
            r.tb.LeftBracket = '(';
            r.tb.RightBracket = ')';
            ...
            r.tb.RightBracket2 = ']';
            Range visiblerange = r.tb.VisibleRange;
            r.ClearStyle(StringStyle, KeywordStyle, KeywordStyle2, KeywordStyle3, NumberStyle);
            visiblerange.SetStyle(CommentStyle, @"//.*$", RegexOptions.Multiline);
            visiblerange.SetStyle(CommentStyle, @"(/\*.*?\*/)|(/\*.*)", RegexOptions.Singleline);
            visiblerange.SetStyle(CommentStyle, @"(/\*.*?\*/)|(.*\*/)",
                RegexOptions.Singleline | RegexOptions.RightToLeft);
        ...
        private void SchemeSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = ";";
            r.tb.LeftBracket = '(';
            r.tb.LeftBracket2 = '[';
            r.tb.RightBracket = ')';
            r.tb.RightBracket2 = ']';
            r.ClearStyle(StringStyle, NumberStyle, PreprocessorStyle, KeywordStyle, KeywordStyle2, KeywordStyle3);
            r.SetStyle(CommentStyle, @"\;.*$", RegexOptions.Multiline);
            r.SetStyle(StringStyle, new Regex(@"""""|''|"".*?[^\\]""|'.*?[^\\]'"));
            r.SetStyle(PreprocessorStyle, @"#\:[a-zA-Z_\d]*\b");
            r.SetStyle(KeywordStyle,
        ...
        private void ShellSyntaxHighlight(Range r)
        {
            r.tb.CommentPrefix = "#";
            r.tb.LeftBracket = '(';
            r.tb.LeftBracket2 = '[';
            r.tb.RightBracket = ')';
            r.tb.RightBracket2 = ']';
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle2, KeywordStyle2, VariableStyle, NumberStyle);
            r.SetStyle(CommentStyle, @"#.*$", RegexOptions.Multiline);
            r.SetStyle(StringStyle, new Regex(@"""""|''|"".*?[^\\]""|'.*?[^\\]'"));
            r.SetStyle(KeywordStyle,
            ...
            r.SetStyle(VariableStyle, @"\b(HOME|PATH|TERM|USER|EXITIN|LOGNAME|MAIL|PWD|SHELL|TERMCAP)\b");
            r.SetStyle(NumberStyle, @"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\b0x[a-fA-F\d]+\b");
        }

        private void MakeFileSyntaxHighlight(Range r)
        {
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle, KeywordStyle2, KeywordStyle3, VariableStyle,
                NumberStyle);
            r.SetStyle(CommentStyle, @"#.*$", RegexOptions.Multiline);
            r.SetStyle(StringStyle, @"""""|''|"".*?[^\\]""|'.*?[^\\]'");
            r.SetStyle(KeywordStyle,
        ...
        private void YamlSyntaxHighlight(Range r)
        {
            r.ClearStyle(CommentStyle, StringStyle, KeywordStyle, NumberStyle);
            r.SetStyle(CommentStyle, @"#.*$", RegexOptions.Multiline);
            r.SetStyle(StringStyle, @"""""|''|"".*?[^\\]""|'.*?[^\\]'");
            r.SetStyle(KeywordStyle, @"^.*?(?=:)", RegexOptions.Multiline);
            r.SetStyle(StringStyle, @"\:.*$", RegexOptions.Multiline);
            r.SetStyle(NumberStyle, @"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\b0x[a-fA-F\d]+\b");
        }

        private void DiffSyntaxHighlight(Range r)
        {
