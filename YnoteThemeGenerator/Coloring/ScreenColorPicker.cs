...
namespace Cyotek.Windows.Forms
{
    ...
    public class ScreenColorPicker : Control, IColorEditor
    {
            ...
            if (e.Button == MouseButtons.Left && !IsCapturing)
            {
                if (_eyedropperCursor == null)
                    ...
                    _eyedropperCursor =
                        new Cursor(
                            GetType()
                                .Assembly.GetManifestResourceStream(string.Concat("YnoteThemeGenerator.Properties",
                                    ".Resources.eyedropper.cur")));
        ...
        /// </summary>
        /// <value>
        ///     <c>true</c> if text is to be shown when a snapshot is present; otherwise, <c>false</c> to only show text when no
        ...
        /// <returns>
        ///     The index value of the control within the set of controls within its container. The controls in the container
        ...
        /// <returns>
        ///     true if the user can give the focus to the control using the TAB key; otherwise, false. The default is
        ///     true.Note:This property will always return true for an instance of the <see cref="T:System.Windows.Forms.Form" />
        ///     class.
        /// </returns>
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        ...
        protected virtual void PaintAdornments(PaintEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(Text) && (!HasSnapshot || ShowTextWithSnapshot))
                TextRenderer.DrawText(e.Graphics, Text, Font, ClientRectangle, ForeColor, BackColor,
                    TextFormatFlags.ExpandTabs | TextFormatFlags.NoPrefix | TextFormatFlags.VerticalCenter |
                    TextFormatFlags.HorizontalCenter | TextFormatFlags.WordBreak | TextFormatFlags.WordEllipsis);
        }
