...
namespace Cyotek.Windows.Forms
{
    ...
    public abstract class PaletteSerializer : IPaletteSerializer
    {
        ...
        public static IPaletteSerializer GetSerializer(string fileName)
        {
            ...
            foreach (
                IPaletteSerializer serializer in
                    SerializerCache.Where(serializer => !(string.IsNullOrEmpty(serializer.DefaultExtension))))
        ...
        private static void CreateFilters()
        {
            ...
            foreach (
                IPaletteSerializer serializer in
                    SerializerCache.Where(
                        serializer =>
                            !(string.IsNullOrEmpty(serializer.DefaultExtension) ||
                              extensions.Contains(serializer.DefaultExtension))).OrderBy(serializer => serializer.Name))
                // ReSharper restore AssignNullToNotNullAttribute
            {
                string extension;
            ...
            _defaultOpenFilter = string.Format("All Supported Palettes ({0})|{0}|{1}|All Files (*.*)|*.*",
                string.Join(";", extensions.ToArray()), sb);
                                ...
                                GetLoadableTypes(assembly)
                                    .Where(
                                        type =>
                                            ...
                                            typeof (IPaletteSerializer).IsAssignableFrom(type))))
            {
                try
                {
        ...
        /// <summary>
        ///     Serializes the specified <see cref="ColorCollection" /> and writes the palette to a file using the specified
        ///     <see cref="Stream" />.
        /// </summary>
